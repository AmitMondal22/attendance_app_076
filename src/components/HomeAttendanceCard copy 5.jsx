...
const HomeAttendanceCard = () => {
  ...
  const check_in = async()=>{
    ...
    if(Object.keys(data.data.data.clock_in_data).length > 0 ){
      ...
      console.log(convertToLocaleTime(data.data.data.clock_in_data.in_time), "****************");
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setCurrentTime(new Date().toLocaleTimeString());
      const specificDate = new Date(2025, 1, 20).toISOString().split("T")[0]; 
      let isodateiomezz=convertToISO(specificDate,currentTime)
      
      if (clockInTime == null){
        var cin=null
      }else{
        var cin=convertToISO(specificDate,clockInTime)

      }
      if(clockOutTime==null){
        var cout = null
      }else{
        var cout=convertToISO(specificDate,clockOutTime)
      }
      calculateTotalHours(cin, cout, isodateiomezz);
    }, 1000);
  ...
  function convertToISO(dateString, timeString) {
    ...
    const [time, period] = timeString.split(" "); // Handle narrow space character
  ...
  function calculateTotalHours(ccin, ccout, ctime) {
    console.log("Calculating total hours...");
    console.log("ccin:", ccin);
    
    console.log("ccout:", ccout);
    // Convert times to Date objects
    const clinn = new Date(ccin);
    const clloutt = ccout ? new Date(ccout) : null;
    const current = new Date(ctime);

    let totalHours = 0;

    if (clloutt && clinn < clloutt) {
        // Case 1: ccin < ccout
        totalHours = (clloutt - clinn) / (1000 * 60 * 60); // Convert ms to hours
    } else {
        // Case 2: ccin > ccout or ccout is empty/null
        totalHours = (current - clinn) / (1000 * 60 * 60);
    }
    console.log("Total hours:", totalHours.toFixed(2)); // Log result rounded to 2 decimal places
    setTotHour(totalHours.toFixed(2));
    return totalHours.toFixed(2); // Return result rounded to 2 decimal places
}
